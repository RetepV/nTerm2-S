...
<sub>Then the problems started. After populating the relay circuits, I could not get them to switch. After lots of measuring and experimenting with an oscilloscope, I could only conclude that the ESP32 was simply not able to source a high enough voltage and that all my calculations and experiments apparently had been wrong. The switching voltage of the relay is 3V. But I found that the ESP32, in my setting, delivered only some 3.15V, and that goes through the BAT43W diode. The diode has a low voltage drop, but it's still something like 0.55V or more at 30mA, and that brings the operating voltage down to 2.61V. Switching one way (with a 1 on the GPIO), it means that the relay gets 2.61V, which is enough to switch it that way. But not the other way. The capacitor is charged with 2.61V. But when it discharges, it discharges through the BC817, which adds another voltage drop of somewhere between 0.1 and 0.4 volt. And now, with worst case 2.21V we are just around the minimum set voltage for the relay, which is 2.25V typical. The charge in the capacitor is not enough to deliver enough energy in that case.

So now I had to experiment with the board itself and found that what I could do was to change the input diode to a transistor in commom collector (emitter follower) configuration, and basically buffer ESP32's GPIO. I.e. replace the BAT43 diode with a BC817 transistor. This brings the voltage up far enough to get the circuit to work. And after fiddling a bit with resistor and capacitor values I got it all working reliably.

I left it at that, but I also had the option of using a common emitter configuration. But I was afraid I would need some more resistors to limit currents.<sub>
