...
<sub>Then the problems started. After populating the relay circuits, I could not get them to switch. After lots of measuring and experimenting with an oscilloscope, I could only conclude that the ESP32 was simply not able to source a high enough voltage and that all my calculations and experiments apparently had been wrong. The switching voltage of the relay is 3V, the ESP32 delivers 3.3V, but that goes through the BAT43W diode. The diode has a low voltage drop, but it's still something like 0.55V or more at 30mA, and that brings the operating voltage down to 2.75V. Presumably enough to switch one way (when the GPIO goes high and the set is done directly from the GPIO current), but not enough to switch the other way (when the GPIO goes low and the set is done from the capacitor, we should not think in current, but more in 'energy' here, and there is just not enough energy at 2.75V). So now I had to experiment with the board itself and found that what I could do is to change the input diode to a transistor, and buffer ESP32's GPIO. I.e. replace the BAT43 diode with a BC817 transistor. This brings the voltage up far enough to get the circuit to work. After fiddling a bit with resistor and capacitor values, I got it all working reliably. But it needed a new board revision.</sub>
